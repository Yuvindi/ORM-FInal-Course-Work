...
public class StudentDAOImpl implements StudentDAO {
    ...
    @Override
    public Student searchByEmail(String id) throws SQLException, ClassNotFoundException {
        Session session = FactoryConfiguration.getInstance().getSession();
        Transaction transaction = session.beginTransaction();
        Student student = session.createQuery("FROM Student WHERE stu_email = :stu_email", Student.class).setParameter("stu_email",id)
                .uniqueResult();
        transaction.commit();
        session.close();
        return student;    }
