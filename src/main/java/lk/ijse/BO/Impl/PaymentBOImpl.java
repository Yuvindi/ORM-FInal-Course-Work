...
PaymentDAO paymentDAO = (PaymentDAO) DAOFactory.getDaoFactory().getDAO(DAOFactory.DaoType.Payment);
    ...
    public boolean save(PaymentDTO dto) throws Exception {
        return paymentDAO.save(new Payment(
                ...
                dto.getStudentCourse().getCourse_batch(),
                new ArrayList<>()
                )));
    }
    ...
    public List<PaymentDTO> getAll() throws SQLException, ClassNotFoundException {
        ...
        for (Payment payment1 : payment) {
            dto.add(new PaymentDTO(payment1.getPay_id(),payment1.getPay_date(),payment1.getPay_amount(),new Student_CourseDTO(payment1.getStudent_course().getStudent_course_id(),new StudentDTO(),new CourseDTO(),payment1.getStudent_course().getRegistration_date(),payment1.getStudent_course().getCourse_batch())));
        }
        return dto;
    }
}
